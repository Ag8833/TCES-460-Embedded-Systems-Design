...
def opencv(conn, conn2):
    ...
    car_cone_cascade = cv2.CascadeClassifier('10x10conecar.xml')
    reload_cone_cascade = cv2.CascadeClassifier('10x10conereload.xml')
    #eye_cascade = cv2.CascadeClassifier('10x10line.xml')
    ...
    try:
        while(1):
            ...
            for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
                ...
                cars = car_cone_cascade.detectMultiScale(gray, 1.05, 1)
                reloads = reload_cone_cascade.detectMultiScale(gray, 1.15, 1)
...
def navigation(conn2):
    ...
    while(1):
        ...
        if imu.IMURead():
            ...
            if(abs(magnet[0]) + abs(magnet[1]) + abs(magnet[2]) > 500):
                BrickPi.MotorSpeed[PORT_A] = 0
                BrickPi.MotorSpeed[PORT_D] = 0
                magFound = True
